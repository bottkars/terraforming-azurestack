...
resource "azurestack_storage_container" "ops_manager_vhd_container" {
  name                  = "vhds"
  depends_on            = ["azurestack_storage_account.ops_manager_storage_account"]
  resource_group_name   = "${var.resource_group_name}"
  storage_account_name  = "${azurestack_storage_account.ops_manager_storage_account.name}"
...
resource "azurestack_virtual_machine" "ops_manager_vm" {
  ...
  storage_os_disk {
    name          = "osdisk.vhd"
    vhd_uri       = "${azurestack_storage_account.ops_manager_storage_account.primary_blob_endpoint}vhds/${azurestack_virtual_machine.ops_manager_vm.name}osdisk.vhd"
...
resource "azurestack_virtual_machine" "optional_ops_manager_vm" {
  ...
  storage_os_disk {
    name          = "optional-osdisk.vhd"
    vhd_uri       = "${azurestack_storage_account.ops_manager_storage_account.primary_blob_endpoint}vhds/${azurestack_virtual_machine.optional_ops_manager_vm.name}-osdisk.vhd"
    image_uri     = "${azurestack_storage_blob.optional_ops_manager_image.url}"
    caching       = "ReadWrite"
    os_type       = "linux"
    create_option = "FromImage"
    disk_size_gb  = "150"
